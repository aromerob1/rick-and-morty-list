...
function CharacterDetailView({ characterId }: CharacterDetailViewProps) {
  const { loading, error, data, refetch: refetchCharacterDetails } = useQuery(GET_CHARACTER_DETAIL_QUERY, {
  ...
  const [addComment, { loading: addingComment, error: addCommentError }] = useMutation(
    ...
    <section className="flex flex-col xl:pt-[2.5rem] xl:px-[6.25rem]">
      ...
      <div className="pt-2 border-t border-gray-300">
          ...
          {comments.length > 0 ? (
            <ul className="space-y-4 overflow-y-auto max-h-28">
              {comments.map((comment) => ( 
                <li key={comment.id} className="bg-gray-50 p-3 rounded-lg shadow-sm">
        ...
        <div className="mt-2 pt-2 border-t border-gray-300">
            <h3 className="text-lg font-semibold mb-3 text-gray-800">Add Comment</h3>
            <form className='pb-12' onSubmit={handleSubmitComment}>
                ...
                />
                {addCommentError && (
                    <p className="text-light-text text-sm mt-1">Error: {addCommentError.message}</p>
                )}
                <button
                    type="submit"
